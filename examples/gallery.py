...
Produces three figures under docs/images/, all from cutouts the package
fetches:
...
def multiwavelength_montage(plt):
    ...
    for ax, (name, band, cmap, data) in zip(axes, grabbed):
        ...
        ax.text(0.5, -0.07, band, transform=ax.transAxes, ha="center", va="top",
                fontsize=9, color="#9aa0a6")
        ax.set_xticks([])
        ax.set_yticks([])
    fig.suptitle("Cygnus A  ·  one source across surveys: radio → infrared → optical",
                 fontsize=13, color="#cfd2d6", y=1.04)
    fig.tight_layout()
...
def spectral_map(plt):
    ...
    cygA = SkyCoord.from_name("Cygnus A")
    tgss, nvss = matched_cutouts(cygA, ["TGSS", "NVSS"], pixels=300, radius=2.2 * u.arcmin)
    alpha = spectral_index_map(
        tgss, SURVEY_FREQUENCY_MHZ["TGSS"], nvss, SURVEY_FREQUENCY_MHZ["NVSS"]
    )
    print(f"  alpha map: median={np.nanmedian(alpha):+.2f}, "
          f"flattest={np.nanmax(alpha):+.2f}, steepest={np.nanmin(alpha):+.2f}")

    fig, (axL, axR) = plt.subplots(1, 2, figsize=(11, 4.6))
    axL.imshow(nvss, origin="lower", cmap="inferno",
               norm=ImageNormalize(nvss, interval=PercentileInterval(99.5),
                                   stretch=AsinhStretch()))
    axL.set_title("Cygnus A — NVSS 1.4 GHz", fontsize=12)
    image = axR.imshow(alpha, origin="lower", cmap="Spectral_r", vmin=-1.3, vmax=0.1)
    axR.contour(nvss, levels=np.nanmax(nvss) * np.array([0.05, 0.2, 0.5, 0.85]),
    ...
    fig.suptitle("Same galaxy, pixel-by-pixel: flat hotspots vs steep lobes",
                 fontsize=12, color="#cfd2d6", y=1.0)
    fig.tight_layout()
...
def calibrator_indices(plt):
    ...
    sample = ["3C48", "3C147", "3C286", "3C295", "3C196", "3C273", "3C84", "M87", "3C348"]
    ...
    sc = ax.scatter(fluxes_n, fluxes_t, c=alphas, cmap="coolwarm_r", s=140,
                    edgecolor="white", vmin=-1.2, vmax=0.2, zorder=3)
    for name, sn, st in zip(names, fluxes_n, fluxes_t):
        ax.annotate(name, (sn, st), fontsize=8, color="#cfd2d6",
                    xytext=(5, 4), textcoords="offset points")
    lim = [min(fluxes_t.min(), fluxes_n.min()) * 0.6,
    ...
    ax.set_title("Radio spectral index from two surveys\n(steep sources sit above the line)",
                 fontsize=11)
...
def main():
    ...
    print("1/3 multi-wavelength montage…")
    multiwavelength_montage(plt)
    print("2/3 spectral-index map…")
    spectral_map(plt)
    print("3/3 calibrator spectral indices…")
